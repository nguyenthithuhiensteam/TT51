...
const state = {
  ...
  publicBranding: { schoolName:'Trường Mầm non Hương Sen', schoolYear:'2026–2027' },
  currentUser: null,
...
function syncPublicBranding(profile = {}) {
  const schoolName = String(profile.schoolName || profile.name || '').trim() || 'Trường Mầm non Hương Sen';
  const schoolYear = String(profile.schoolYear || '').trim() || '2026–2027';
  state.publicBranding = { schoolName, schoolYear };
  document.title = `${schoolName} - CTGDMN`;
}
...
function currentPlanFromForm() {
  ...
  return {...Object.fromEntries(formData),collaboratingTeacherIds:formData.getAll('collaboratingTeacherIds')};
}
...
function rowsToEditorFields(level,rows=[]) {
  ...
  if(type==='lesson'){const first=rows[0]||{};return{teacherActivities:first.teacherActivity||first.activity||'',childActivities:first.childActivity||''};}
...
function applyAIResult() {
  ...
  const result=payload.result;
  const values={title:result.title,context:result.context,objectives:result.objectives,activities:result.activities,materials:result.materials,differentiation:result.differentiation,assessment:result.assessment,family:result.family,...rowsToEditorFields(form.elements.level.value,result.templateRows)};
...
function planTemplate(plan = {}) {
  ...
  return `<form id="plan-form" class="plan-form">
    ...
    <details class="template-editor" ${plan.level==='Ngày/hoạt động'?'open':''}><summary>Mẫu giáo án/hoạt động giáo dục ngày</summary>
      ...
      <div class="form-grid"><label class="field"><span>Hoạt động của cô</span><textarea class="tall" name="teacherActivities">${escapeHtml(plan.teacherActivities||'')}</textarea></label><label class="field"><span>Hoạt động của trẻ</span><textarea class="tall" name="childActivities">${escapeHtml(plan.childActivities||'')}</textarea></label></div>
...
function authScreen(kind, message = '') {
  const school=state.publicBranding.schoolName;const setup=kind==='setup';syncPublicBranding(state.publicBranding);
  els.loading.className='loading-screen auth-screen';els.loading.innerHTML=`<section class="auth-card"><img src="../assets/icon.png" alt="Logo DNA Hiền"><p class="eyebrow">${setup?'THIẾT LẬP LẦN ĐẦU':'ĐĂNG NHẬP NỘI BỘ'}</p><h1>${escapeHtml(school)}</h1><p>${setup?'Tạo tài khoản quản trị đầu tiên. Ứng dụng không có tài khoản hoặc mật khẩu mặc định.':'Dữ liệu được lưu cục bộ trong SQLite trên máy này.'}</p>${message?`<div class="auth-error">${escapeHtml(message)}</div>`:''}<form id="${setup?'first-admin-form':'login-form'}" class="plan-form">${setup?'<label class="field"><span>Họ và tên *</span><input class="input" required name="fullName" autocomplete="name"></label>':''}<label class="field"><span>Tên đăng nhập *</span><input class="input" required name="username" autocomplete="username"></label><label class="field"><span>Mật khẩu *</span><div class="password-field"><input class="input" required minlength="8" name="password" type="password" autocomplete="${setup?'new-password':'current-password'}"><button type="button" data-toggle-password>Hiện</button></div></label>${setup?'<label class="field"><span>Nhập lại mật khẩu *</span><input class="input" required minlength="8" name="confirmPassword" type="password" autocomplete="new-password"></label>':''}<button class="primary-button" type="submit">${setup?'Tạo quản trị đầu tiên':'Đăng nhập'}</button></form><small>CTGDMN phiên bản 0.6.0 • Hoạt động không cần Internet</small></section>`;
